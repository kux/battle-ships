...
ShipPattern::ShipPattern( const std::string& sShipTypeName, const CoordinateListType& objCoord )
throw (Exception):
...
CoordinateListType ShipPattern::getAbsolutePozitions( const CoordinateType& coord,
	Orientation orientation ) const
{
	...
	for( CoordinateListType::const_iterator i = coordList_.begin() ;
		i != end; ++i ) {

		switch ( orientation )
		{
		case rot0: {
				CoordinateType newCoord ( (*i).first + coord.first, (*i).second + coord.second );
				newCoordsList.push_back( newCoord );
				break;
			}
		case rot90: {
				CoordinateType newCoord ( (*i).second + coord.first, -(*i).first + coord.second );
				newCoordsList.push_back( newCoord );
				break;
			}
		case rot180:{
...
bool ShipPattern::validatePattern( const CoordinateListType& objCoord_)
{
	...
	for ( CoordinateListType::const_iterator i = objCoord_.begin();
		i != objCoord_.end(); ++i )
	{
		CoordinateListType::const_iterator frez = find ( i + 1, objCoord_.end(), *i );
		if ( frez != objCoord_.end() ) throw Exception( Exception::DUPLICATE_PATTERN );
	}
	...
	{
		if ( !vHasAdiacent[i] ) throw Exception( Exception::NOTCONEX_PATTERN );
	}
